...
function detectLocale(req: NextRequest) {
  ...
  console.log(accept, first, short);
  return [first, short].find((l) => i18n.locales.includes(l)) ?? i18n.defaultLocale;
}
...
export default clerkMiddleware(async (auth, req: NextRequest) => {
  ...
  const chosen = saved && i18n.locales.includes(saved) ? saved : detectLocale(req);
