...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Cake } from 'lucide-react';
 ...
 */
function parseTimeProp(time: string): { hours: number; minutes: number; seconds: number } | null {
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        month:
          'flex flex-col items-center space-y-4 [&_.rdp-caption]:flex [&_.rdp-caption]:w-full [&_.rdp-caption]:justify-between [&_.rdp-caption]:items-center',
        ...
        <Popover>
          <PopoverTrigger asChild disabled={disabled}>
            <Button
              ...
              ref={buttonRef}
            >
              {mode !== 'date' && mode !== 'time' && mode !== 'datetime' && !hideIcon ? (
                ...
                <TimePicker
                  onChange={(value) => {
                    if (value && time) {
                      ...
                      const parsed = parseTimeProp(time);
                      if (parsed) {
                        updatedDate.setHours(parsed.hours, parsed.minutes, parsed.seconds, 0);
                      }
